
protCD = ({"UUU", "UUC"}, {"UUA", "UUG"}, {"UCU", "UCC", "UCA", "UCG"}, {"UAU", "UAC"}, {"UAA", "UAG"}, {"UGU", "UGC"}, {"UGA"}, {"UGG"}, {"CUU", "CUC", "CUA", "CUG"}, {"CCU", "CCC", "CCA", "CCG"}, {"CAU", "CAC"}, {"CAA", "CAG"}, {"CGU", "CGC", "CGA", "CGG"}, {"AUU", "AUC", "AUA"}, {"AUG"}, {"ACU", "ACC", "ACA", "ACG"}, {"AAU", "AAC"}, {"AAA", "AAG"}, {"AGU", "AGC"}, {"AGA", "AGG"}, {"GUU", "GUC", "GUA", "GUG"}, {"GCU", "GCC", "GCA", "GCG"}, {"GAU", "GAC"}, {"GAA", "GAG"}, {"GGU", "GGC", "GGA", "GGG"})
protRS = ("Phe", "Leu", "Ser", "Tyr", "[END]", "Cys", "[END]", "Trp", "Leu", "Pro", "His", "Gln", "Arg", "Ile", "[MET]", "Thr", "Asn", "Lys", "Ser", "Arg", "Val", "Ala", "Asp", "Glu", "Gly")


def corregir(texto):
    #elimina los espacios

    texto2 = list(texto)
    while texto.find(" ") != -1:
        texto2.pop(texto.find(" "))
        texto = ''.join(texto2)

    return texto.upper()

def trans(secuencia):
    # Convertimos a mayúsculas por seguridad
    secuencia = secuencia.upper()
    
    # La transcripción cambia la T por la U
    arn = secuencia.replace("T", "U")
    return arn

# --- Bloque de entrada con validación ---
try:
    secuencia_adn = input("Introduzca la secuencia: ").strip().upper()
    
    # Validación simple: que solo contenga A, C, G, T
    if all(base in "ACGT" for base in secuencia_adn):
        resultado_arn = trans(secuencia_adn)
        print(f"Secuencia ARN: {resultado_arn}")
    else:
        print("Error: La secuencia contiene caracteres no válidos para ADN.")

except Exception as e:
    print(f"Error introduciendo el tipo de valor: {e}")

def dividirEnTres(texto):
    #divide la entrada en una lista de tres en tres caracteres
    texto = corregir(texto)
    resultado = [texto[i:i + 3] for i in range(0, len(texto), 3)]
    return resultado

def encontrarProt(texto):

    ARN_texto = dividirEnTres(tipo(texto, "ARNm"))
    ARN_texto = ARN_texto[0]

    for i in range(len(protRS)):
        if ARN_texto in protCD[i]:
            return protRS[i]

    return "Ninguna"

def proteinList(texto):
    texto = corregir(texto)
    # Si el usuario introduce algo como "GAC", no asumamos que es ADN
    # Solo transformamos si estamos seguros de que es ADN (tiene T)
    if "T" in texto:
        texto = trans(texto) # Convertir ADN a ARN
    
    ARN_List = dividirEnTres(texto)
    proteinas = []

    for ARN in ARN_List:
        # Simplificamos: encontrarProt ahora recibe ARN directamente
        res = encontrarProtDirecto(ARN)
        proteinas.append(res)

    return ' '.join(proteinas)

def encontrarProtDirecto(codon):
    # Buscamos directamente en las listas protCD
    for i in range(len(protCD)):
        if codon in protCD[i]:
            return protRS[i]
    return "Ninguna"

def tipo(texto, darTipo="tipo"):

    texto = corregir(texto)

    if texto.find("U") != -1:
        if darTipo == "tipo":
            return "ARN"
        elif darTipo == "ADN":
            return trans(texto)
        else:
            return texto

    else:
        if darTipo == "tipo":
            return "ADN"
        elif darTipo == "ARN":
            return trans(texto)
        else:
            return texto

def encontrarCodigo(texto):
    if texto in protRS:
        iter = 0
        for valor in protRS:
            if valor == texto:
                return ' '.join(protCD[iter])
                break
            else:
                iter += 1
    else:
        return "Ninguno"

def codigoList(texto, tipo="ADN"):

    codeList = texto.split(" ")
    codigos = []

    for prot in codeList:
        codigos.append(encontrarCodigo(prot))

    if tipo == "ARN":
        return ' '.join(codigos)
    else:
        return trans(''.join(codigos))

def Datos(texto):
    print("Tipo:", tipo(texto))
    print("---------------------------------------------")
    print("ADN: ", tipo(texto, "ADN"))
    print("ARN:", tipo(texto, "ARNm"))
    print("---------------------------------------------")
    print("\nProteinas correspondientes:", proteinList(texto))
    print("\n///////////////////////////////////////////\n")
    return ""


def Test():
    import time
    tiempoInicial = time.time()
    print("trans() test: passed?:", trans("ATG")=="UAC", "expected output:", "UAC actual output:", trans("ATG"))
    print("dividirEnTres() test: passed?:", dividirEnTres("AAAAAA")==["AAA", "AAA"], "expected output: [\"AAA\", \"AAA\"] actual output:", dividirEnTres("AAAAAA"))
    print("encontrarProt() test: passed?:", encontrarProt("AAA")=="Lys", "expected output: Lys actual output:", encontrarProt("AAA"))
    print("proteinList() test passed?:", proteinList("AAATTT")=="Phe Lys", "expected output: Phe, Lys actual output:", proteinList("AAATTT"))
    print("tipo() test passed?:", tipo("uuu")=="ARN" and tipo("ttt")=="ADN", "expected output: ARN, ADN actual output:", tipo("uuu"), ",", tipo("ttt"))
    print("encontrarCodigo() test passed?:", encontrarCodigo("Phe")=="AAA", "expected output: AAA actual output:", encontrarCodigo("Phe"))
    print("codigoList() test passed?:", codigoList("Phe Lys")=="AAATTT", "expected output: AAATTT actual output:", codigoList("Phe Lys"))
    print(time.time() - tiempoInicial)


def ayuda():
    print("Ver. 1.0.0"
          "trans(Codigo) -> Si el valor introducido es ADN lo transforma a ARNm y viceversa"
          "dividirEnTres(Codigo) -> Divide en bloques un codigo de ADN o ARNm. Devuelve una lista."
          "encontrarProt(Codigo) -> Encuentra UNA proteina de un bloque de codigo (se convierte"
          "                         a ARNm de forma automatica)."
          "proteinList(Codigo) -> Transforma una tira de ADN (o varios ARNm en una sola tira) en"
          "                       todas las proteinas que contiene esa informacion."
          "tipo(Codigo, tipo[opcional]) -> Comprueba si el valor introducido es ADN o ARNm"
          "                                En caso de tener una segunda entrada traduce el codigo"
          "                                en un tipo especifico indicado por la segunda entrada."
          "                                Si no se puede determinar el tipo, devolvera ADN por defecto."
          "encontrarCodigo(Proteina) -> Indica el/los codigo(s) necesario(s) para obtener una proteina"
          "                             especifica (ARNm)."
          "codigoList(Proteinas, tipo[opcional]) -> Hace una lista del ADN necesario para obtener unas"
          "                                         proteinas especificas.\n"
          "                                         El tipo (opcional), por defecto es ADN, aunque se"
          "                                         puede seleccionar ARNm. En caso de ser ARNm, el"
          "                                         resultado viene separado por espacios."
          "Datos(Codigo) -> indica todos los datos de el codigo en la terminal")

help = ayuda
